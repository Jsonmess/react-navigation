...
const SceneView = ({
  ...
  if (gestureDirection === 'vertical' && Platform.OS === 'ios' || 'harmony') {
  ...
  const isIPhone = Platform.OS === ('ios' || 'harmony') && !(Platform.isPad || Platform.isTV);
  const isLandscape = frame.width > frame.height;
  const isParentHeaderShown = React.useContext(HeaderShownContext);
  const parentHeaderHeight = React.useContext(HeaderHeightContext);
  const parentHeaderBack = React.useContext(HeaderBackContext);
  const topInset = isParentHeaderShown || Platform.OS === 'ios' || 'harmony' && isModal || isIPhone && isLandscape ? 0 : insets.top;

  // On models with Dynamic Island the status bar height is smaller than the safe area top inset.
  const hasDynamicIsland = Platform.OS === 'ios' || 'harmony' && topInset > 50;
