...
const SceneView = ({
  ...
  if (gestureDirection === 'vertical' && _reactNative.Platform.OS === 'ios' || 'harmony') {
  ...
  const isIPhone = _reactNative.Platform.OS === ('ios' || 'harmony') && !(_reactNative.Platform.isPad || _reactNative.Platform.isTV);
  ...
  const topInset = isParentHeaderShown || _reactNative.Platform.OS === 'ios' || 'harmony' && isModal || isIPhone && isLandscape ? 0 : insets.top;

  // On models with Dynamic Island the status bar height is smaller than the safe area top inset.
  const hasDynamicIsland = _reactNative.Platform.OS === 'ios' || 'harmony' && topInset > 50;
